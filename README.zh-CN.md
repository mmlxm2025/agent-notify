...
一个面向 AI Agent 的通知配置工具。支持将 Claude Code、Codex、ZCode (Z.ai) 等 Agent 的事件通知推送到飞书、企业微信、微信兼容（自建转发）、钉钉、Bark、ntfy、Slack 和系统通知。
...
| <img src="assist/logo/qiyeweixin.png" width="24" align="absmiddle"> 微信兼容 | 面向只接受 `{title, content}` 格式的自建/中转 Webhook 服务（如个人搭建的转发到企业微信的推送网关） | Webhook |
...
原生的**企业微信**渠道发送企业微信标准的 `{msgtype, markdown}` 格式，供官方企业微信群机器人使用。但部分自建/中转的转发服务只接受 `{title, content}` 格式的 JSON——向它们发送标准格式会返回 `400 "消息内容不能为空"`。**微信兼容**渠道即面向这类服务。

1. **准备自建 Webhook**：获取你的自建转发服务 URL，要求接收 `{title, content}` 形式的 POST JSON（例如 `https://your-server.com/api/notify/<key>`）。
2. **绑定配置**：运行 `npx agent-notify`，进入「消息渠道配置」-> 选择「微信兼容」，粘贴 Webhook URL 即可。
3. **内容格式**：`content` 字段以**纯文本**发送（去除 `##`、`>`、`**`、`` ` ``、`<font>` 等 markdown 语法符号），因为这类转发服务通常不渲染 markdown。

Webhook URL 会作为本地配置保存，默认对所有 Agent 生效。
